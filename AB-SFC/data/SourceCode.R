...
simulateAB<-function(phi_baseline = 1, alpha11_baseline = 0.4, alpha12_baseline = 0.25, alpha2_baseline = 0.4, theta_baseline = 0.2, sigmap_baseline = 0.00, sigmaw_baseline = 0.00, G_baseline = 20, phi_shock = 1, 
										 alpha11_shock = 0.4, alpha12_shock = 0.25, alpha2_shock = 0.4, theta_shock = 0.2, sigmap_shock = 0.00, sigmaw_shock = 0.00, G_shock = 10, shock = 0,incomeDistr=FALSE){
	...
	for (mc in 1:MC) {
		...
		res_mc = as.data.frame(matrix(0,ncol=15,nrow=Time,dimnames = list(NULL,c("CY","G","C","WB","Y","LF","UR","Q","PROF","YD","H","TAX","PDef","RATIONING","RATLIQ"))))
		...
		for (t in 1:Time) {
			...
			if (mc==1 | (shock>0 & t <= shock)) {
				phi = phi_baseline
				cy = alpha11_baseline*yd+alpha12_baseline
				ch = alpha2_baseline
			...
			c = cy * yd/pHh + ch * h/pHh
			# We assume that households can only consume in units, because of that, rationing exists
			c = floor(c)
			rationing = y * yd + ch * h - c
			res_mc$RATIONING[t]=sum(rationing)
			
			# H look at firms' prices randomly (uniform distribution, default value over the [0,1] interval), price play no role here
			# indpf
			...
			yd = (1 - theta) * y
			res_mc$YD[t] = sum(yd)
			res_mc$TAX[t] = sum(theta * y)
			h = h + yd - c
			res_mc$H[t] = sum(h)
			
			# Public deficit
			res_mc$PDef[t] = res_mc$G[t] - res_mc$TAX[t]
